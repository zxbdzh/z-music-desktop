...
export const onSettingChanged = (
  listener: LX.IpcRendererEventListenerParams<Partial<LX.DesktopLyric.Config>>
): RemoveListener => {
  rendererOn<Partial<LX.DesktopLyric.Config>>(
    WIN_LYRIC_RENDERER_EVENT_NAME.on_config_change,
    listener
  )
  return () => {
...
export const onProvideMainWindowChannel = (
  listener: LX.IpcRendererEventListener
): RemoveListener => {
