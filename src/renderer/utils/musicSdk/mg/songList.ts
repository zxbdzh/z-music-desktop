...
export default {
  ...
  limit_song: 50,
  successCode: '000000',
  cachedDetailInfo: {} as Record<string, any>,
  cachedUrl: {} as Record<string, string>,
  sortList: [
    {
      name: '推荐',
      id: '15127315',
  ...
  getListDetailList(id: string, page: number, tryNum: number = 0): Promise<any> {
    ...
    return requestObj_listDetail.promise.then(({ body }: any) => {
  ...
  getListDetailInfo(id: string, tryNum: number = 0): Promise<any> {
    if (tryNum > 2) return Promise.reject(new Error('try max num'))
    ...
    return requestObj_listDetailInfo.promise.then(({ body }: any) => {
  ...
  filterList2(listData: any[], list: any[] = [], ids: Set<string> = new Set()): any[] {
