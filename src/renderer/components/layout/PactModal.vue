<template>
  <material-modal
    :show="!isAgreePact || isShowPact"
    max-width="70%"
    :bg-close="isAgreePact"
    :close-btn="isAgreePact"
    @close="handleClose(false)"
  >
    <main :class="$style.main">
      <h2>许可协议</h2>
      <div class="select scroll" :class="$style.content">
        <template v-if="!isAgreePact"
          ><p><strong>在使用本软件前，你（使用者）需签署本协议才可继续使用！</strong></p>
          <br
        /></template>
        <p>
          本项目基于&nbsp;<strong
            class="hover underline"
            @click="openUrl('http://www.apache.org/licenses/LICENSE-2.0')"
            >Apache License 2.0</strong
          >&nbsp;许可证发行。以下协议是对于 Apache License 2.0 的补充，如有冲突，以以下协议为准。
        </p>
        <br />
        <p>
          词语约定：本协议中的“本项目”指 z-music-desktop 桌面音乐客户端；“使用者”指签署本协议的使用者；“官方音乐平台”指对本项目内置的包括酷我、酷狗、咪咕等音乐源的官方平台统称；“版权数据”指包括但不限于图像、音频、名字等在内的他人拥有所属版权的数据。
        </p>
        <br />
        <p><strong>一、数据来源</strong></p>
        <br />
        <p>
          1.1&nbsp;本项目的各官方平台在线数据来源原理是从其公开服务器中拉取数据（与未登录状态在官方平台
          APP
          获取的数据相同），经过对数据简单地筛选与合并后进行展示，因此本项目不对数据的合法性、准确性负责。
        </p>
        <br />
        <p>
        ...
        <br />
        <p>
          2.1&nbsp;使用本项目的过程中可能会产生版权数据。对于这些版权数据，本项目不拥有它们的所有权。为了避免侵权，使用者务必在&nbsp;<strong>24&nbsp;小时内</strong>&nbsp;清除使用本项目的过程中所产生的版权数据。
        </p>
        <br />
        ...
        <p>
          6.1&nbsp;本项目完全免费，且开源发布于&nbsp;<span
            class="hover underline"
            @click="openUrl('https://github.com/zxbdzh/z-music-desktop#许可证')"
            >GitHub</span
          >&nbsp;面向全世界人用作对技术的学习交流，本项目不对项目内的技术可能存在违反当地法律法规的行为作保证。
        </p>
        <br />
        <p>
          6.2&nbsp;<strong>禁止在违反当地法律法规的情况下使用本项目。</strong>&nbsp;对于使用者在明知或不知当地法律法规不允许的情况下使用本项目所造成的任何违法违规行为由使用者承担，本项目不承担由此造成的任何直接、间接、特殊、偶然或结果性责任。
        </p>
        <br />
        <p><strong>七、版权保护</strong></p>
        <br />
        <p>7.1&nbsp;音乐平台不易，请尊重版权，支持正版。</p>
        <br />
        ...
        <br />
        <p>9.1&nbsp;若你使用了本项目，即代表你接受本协议。</p>
        <br />
        <p><strong>*</strong>&nbsp;若协议更新，恕不另行通知，可到开源地址查看。</p>
        <p v-if="!isAgreePact">
          <strong
            >若你（使用者）接受以上协议，请点击下面的“接受”按钮签署本协议，若不接受，请点击“不接受”后退出软件并清除本软件的所有数据。</strong
          >
        </p>
      </div>
      <div v-if="!isAgreePact" :class="$style.btns">
        <base-btn :class="$style.btn" @click="handleClose(true)">{{ $t('not_agree') }}</base-btn>
        <base-btn :class="$style.btn" :disabled="!btnEnable" @click="handleClick"
          >{{ $t('agree') }} {{ timeStr }}</base-btn
        >
      </div>
    </main>
  </material-modal>
