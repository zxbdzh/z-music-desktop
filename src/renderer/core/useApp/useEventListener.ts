...
const handle_key_down = ({ event, type, key }: LX.KeyDownEevent) => {
  ...
  if (
    ...
    event.repeat ||
    type == 'up' ||
    window.lx.isEditingHotKey ||
    (event.target as HTMLElement)?.classList.contains('ignore-esc') ||
    event.lx_handled
  )
    return
...
const handleBodyClick = (event: MouseEvent) => {
  ...
  if (/^https?:\/\//.test((event?.target as HTMLAnchorElement).href))
    void openUrl((event?.target as HTMLAnchorElement).href)
}
