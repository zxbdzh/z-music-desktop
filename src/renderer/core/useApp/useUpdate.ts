...
export default () => {
    ...
    void getLastStartInfo().then((version) => {
      ...
      if (version) {
        if (compareVer(process.versions.app, version) < 0) {
          void dialog({
            message: window.i18n.t('update__downgrade_tip', {
              ver: `${version} → ${process.versions.app}`,
            }),
        ...
        ![
          { version: versionInfo.newVersion!.version, desc: '' },
  ...
  const handleGetVersionInfo = async (): Promise<
    NonNullable<(typeof versionInfo)['newVersion']>
  > => {
    return (
      versionInfo.newVersion?.history && !versionInfo.reCheck
        ...
        return getIgnoreVersion().then((ignoreVersion) => {
          ...
          void nextTick(() => {
            ...
            if (
              ...
              !localStorage.getItem('update__download_failed_tip')
            ) {
              setTimeout(() => {
                void dialog({
