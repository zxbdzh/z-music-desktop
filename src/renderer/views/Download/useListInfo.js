...
export default (activeTab) => {
  ...
  const list = computed(() => {
    switch (activeTab.value) {
      case 'runing':
        return listAll.value.filter(
          (i) => i.status == downloadStatus.RUN || i.status == downloadStatus.WAITING
        )
      case 'paused':
  ...
  const playTaskId = computed(() =>
    playMusicInfo.listId == LIST_IDS.DOWNLOAD ? listAll.value[playInfo.playIndex]?.id : ''
  )
