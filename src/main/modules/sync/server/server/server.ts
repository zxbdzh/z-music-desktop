...
const handleConnection = async (socket: LX.Sync.Server.Socket, request: IncomingMessage) => {
  ...
  socket.onClose(() => {
    status.devices.splice(
      status.devices.findIndex((k) => k.clientId == keyInfo.clientId),
      1
    )
    sendServerStatus(status)
  })
...
const handleStartServer = async (port = 9527, ip = '0.0.0.0') =>
    ...
    wss.on('connection', function (socket, request) {
      ...
      const msg2call = createMsg2call<LX.Sync.ClientSyncActions>({
        ...
        sendMessage(data) {
          ...
          void encryptMsg(socket.keyInfo, JSON.stringify(data))
            .then((data) => {
