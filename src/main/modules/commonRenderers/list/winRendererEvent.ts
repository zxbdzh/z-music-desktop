...
export const registerRendererEvents = (
  ...
  const list_update_position = async (position: number, ids: string[]) => {
    sendEvent<LX.List.ListActionUpdatePosition>(PLAYER_EVENT_NAME.list_update_position, {
      position,
      ids,
    })
  }
  const list_music_add = async (
    ...
    sendEvent<LX.List.ListActionMusicAdd>(PLAYER_EVENT_NAME.list_music_add, {
      id,
      musicInfos,
  ...
  const list_music_move = async (
    ...
    sendEvent<LX.List.ListActionMusicMove>(PLAYER_EVENT_NAME.list_music_move, {
      fromId,
      toId,
      musicInfos,
      addMusicLocationType,
  ...
  const list_music_update_position = async (listId: string, position: number, ids: string[]) => {
    sendEvent<LX.List.ListActionMusicUpdatePosition>(PLAYER_EVENT_NAME.list_music_update_position, {
      listId,
      position,
      ids,
    })
  ...
  const list_music_overwrite = async (listId: string, musicInfos: LX.Music.MusicInfo[]) => {
    sendEvent<LX.List.ListActionMusicOverwrite>(PLAYER_EVENT_NAME.list_music_overwrite, {
      listId,
      musicInfos,
    })
  }
