...
export default () => {
  ...
  mainHandle<LX.List.ListActionMusicMove>(
  ...
  mainHandle<LX.List.ListActionMusicOverwrite>(
  ...
  mainHandle<LX.List.ListActionMusicClear>(
    PLAYER_EVENT_NAME.list_music_clear,
    async ({ params: listId }) => {
      await global.lx.event_list.list_music_clear(listId, false)
    }
  )
  mainHandle<LX.List.ListActionCheckMusicExistList, boolean>(
