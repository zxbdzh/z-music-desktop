  ...
  new Promise((resolve, reject) => {
    cos.getBucket(
      ...
      function (err, data) {
        ...
        resolve(
          data.Contents.filter((o) => o.Key !== config.prefix).map((o) =>
            o.Key.replace(config.prefix, '')
          )
        )
      }
    )
...
const createProgressBar = (name, spacekLen, total) =>
  multi.newBar(
